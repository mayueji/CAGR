from time import time
import argparse
import os
import numpy as np
import trimesh
import os



def find_nearest_points_and_compute_inner_product(points1, normals1, points2, normals2):
  
    kdtree = KDTree(points2)

    _, indices = kdtree.query(points1)


    inner_products = np.sum(normals1 * normals2[indices], axis=1)

    return inner_products
def compute_shortest_distances(point_cloud_A, point_cloud_B):
    kdtree_B = cKDTree(point_cloud_B)  

    distances, _ = kdtree_B.query(point_cloud_A)  
    return distances

def isexist(name, path=None):

    if path is None:
        path = os.getcwd()
    if os.path.exists(path + '/' + name):
        print("Under the path: " + path + '\n' + name + " is exist")
        return True
    else:
        if (os.path.exists(path)):
            print("Under the path: " + path + '\n' + name + " is not exist")
        else:
            print("This path could not be found: " + path + '\n')
        return False

parser = argparse.ArgumentParser()
parser.add_argument('input', help='ply format point cloud input, this should be the one to construct the final query octree')
parser.add_argument('-c', '--direction', type=float, default=0.5, help='direction for the  anisotropic guass formula')
args = parser.parse_args()
setup_str = f'---------Settings---------\n'\
          + f'input:   {args.input}\n'\
          + f'---------Settings---------'

print(setup_str)


N=5000
i=0
t=0.005
N_p=100000
NC=0
CD=0
FS=0
PGP=0
currentpath = os.getcwd().replace("\\",'/') 


point= os.path.join(currentpath,'point').replace("\\",'/')




file_name=args.input
name=os.path.basename(file_name)
mesh = trimesh.load_mesh(file_name)
points,faces=trimesh.sample.sample_surface_even(mesh,N)
mu, sigma = 0, 1 
s = np.random.normal(mu, sigma, points.shape) *0.000
normal=mesh.face_normals[faces]
PointCloud=np.array(points)+s
np.savetxt(point+'/'+name[:-4]+'_5K.xyz',  np.c_[PointCloud],fmt='%f',delimiter=' ')
sample=point+'/'+name[:-4]+'_5K.xyz'
A=r"python run_AGR.py   --alpha 2 -wk 16 "
recon_cmdPGR= f"{A} "+ f"-c {args.direction} "+ f"{sample}"
os.system(recon_cmdPGR)
